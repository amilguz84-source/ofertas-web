...
def extract_description(text: str, title: str, max_chars: int = 220) -> str:
    """Recoge las líneas de texto que no son el título, un enlace, un código
    de cupón o solo hashtags, para mostrarlas como descripción del producto."""
    parts = []
    for line in text.split("\n"):
        if URL_RE.search(line):
            continue
...
def parse_message(msg_div, session: requests.Session, resolve_links: bool) -> dict | None:
    ...
    # Última red de seguridad: si el link no venía como <a> clicable (a veces
    # Telegram no lo detecta como enlace y queda como texto plano), lo buscamos
    # directamente en el texto del mensaje con una expresión regular.
    if not raw_link:
        url_match = URL_RE.search(text)
        if url_match:
            raw_link = url_match.group(0).rstrip(").,;:")
