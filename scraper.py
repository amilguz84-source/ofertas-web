...
def extract_title(text: str) -> str:
    """Busca la primera línea con contenido real (descarta líneas que son solo emojis)."""
...
def parse_message(msg_div, session: requests.Session, resolve_links: bool) -> dict | None:
    ...
    # El enlace de compra suele venir como botón bajo el post ("Ver oferta"),
    # no como un <a> dentro del texto. Si no hay botón, usamos el primer
    # enlace que aparezca en el texto como alternativa.
    ...
    raw_link = button_links[0] if button_links else (text_links[0] if text_links else None)
